#!/bin/bash
printf "nazwa_uzytkownika np. user_01\n"
read new_user
printf "Nazwa uzytkownika to: %s\n" "$new_user"
printf "Jaka sciezke ten uzytkownik powinien uzywac jako katalog domowy np. /home/nazwa_uzytkownika\n"
read user_home_path
printf "Sciezka tego uzytkownika to: %s\n" "$user_home_path"
if test -z "$new_user" || test -z "$user_home_path"; then
    ...
    if id "$new_user" &>/dev/null; then
        ...
        printf "Jakie haslo chcesz ustawic dla swojego uzytkownika?\n"
        read  haslo
